...
): Promise<AccountData<CertificateData>[]> {
  const certificatesIds = await Promise.all(
    mints.map((mint) => certificateIdForMint(mint)[0])
  );
...
export async function getOutstandingCertificates(
  ...
  const coder = new anchor.BorshAccountsCoder(CERTIFICATE_IDL);
  ...
  programAccounts.forEach((account) => {
    try {
      ...
      const certificate = coder.decode("certificate", account.account.data);
      if (
