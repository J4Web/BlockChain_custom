...
export const useAddressImage = (connection, address, namespaceName = TWITTER_NAMESPACE_NAME, dev) => {
    const [addressImage, setAddressImage] = useState(undefined);
    const [loadingImage, setLoadingImage] = useState(true);
    const { displayName, loadingName } = useAddressName(connection, address, namespaceName);
    const refreshImage = (displayName) => __awaiter(void 0, void 0, void 0, function* () {
        try {
            ...
            if (handle) {
                const imageUrl = yield tryGetImageUrl(namespaceName, handle, dev);
